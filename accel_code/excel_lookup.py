...
def excel_lookup(lab_id, date, excel_file):
    ...
    try:
        ...
        if len(ses.index) > 1:
            raise IndexError(
                ("{lab_id}: {date} has multiple entries in excel file"
                    .format(lab_id=lab_id, date=date))
                )
        ...
        elif project == 'BIKE_Pre':
            ses_id = 'pre'

        elif project == 'BIKE_Post':
            ses_id = 'post'
        ...
        raise ValueError(
                ("{lab_id}: {date} is not found in excel file"
                    .format(lab_id=lab_id, date=date))
                )
